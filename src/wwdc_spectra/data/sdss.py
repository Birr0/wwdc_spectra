...
class SDSS(Dataset):
    ...
    def prepare_spectrum(
        ...
        start = int(torch.round((loglam[0] - torch.log10(self._wave_obs[0]).item())/0.0001))
        if start<0:
            flux = flux[-start:]
            ivar = ivar[-start:]
    ...
    def _to_f32_or_nan(x):
        return torch.tensor(float(x) if x is not None else float('nan'),
                        dtype=torch.float32)
    ...
    def __getitem__(self, idx):
        ...
        y = torch.stack(
            [
                ...
                for k in self.y_catalog.variables if k not in self.y_catalog["drop_variables"]
            ]
        )
        if self.return_id:
...
class SDSS_AION(SDSS):
    def __getitem__(self, idx):
        ...
        return item["spectrum"]["flux"], item["spectrum"]["ivar"], \
            item["spectrum"]["lambda"], item["spectrum"]["mask"], y
