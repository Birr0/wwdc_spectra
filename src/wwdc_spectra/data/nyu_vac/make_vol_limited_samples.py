...
def main():
    ...
    for split in splits:
        ...
        mask_z_agree = np.isclose(z_k, z_r, atol=Z_TOL, rtol=0)

        mask_box = (z > 0) & (z <= z_cut) & (M_r <= M1).reshape(-1) & mask_z_agree
    ...
    ds_dict.save_to_disk(
        f"{DATA_ROOT}/sdss/volume_limited/z={z_cut.item():.3f}_spectra"
    )
    ds_dict.push_to_hub(
        f"{HF_USERNAME}/vol_limited_z_{z_cut.item():.3f}_spectra",
        private=False, 
    )
    print(f"Pushed dataset: {z_cut.item()}")
