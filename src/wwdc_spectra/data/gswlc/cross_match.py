...
def process_embeddings(
    ...
    df = pd.DataFrame({
        ...
        "ObjID": [_norm_id(x) for x in embedding_data["BESTOBJID"]],
...
def match_catalog_to_embeddings(
    ...
    df = picked[['ObjID', 'z', 'z_batch', 'rchi2', 'logM*', 'logM*_err', 'logSFR',
...
def merge_matched_cat_with_embeddings(
    ...
    #print(matched_cat["id"])

    data_files = {
        "train": f"{embeddings_fp}/train/*.parquet",
        "test": f"{embeddings_fp}/test/*.parquet",
        "val": f"{embeddings_fp}/val/*.parquet"
    }
    ...
    embeddings["BESTOBJID"] = embeddings["BESTOBJID"].map(_norm_id)
   
    assert np.array_equal(embeddings["BESTOBJID"].values, matched_cat["id"].values), "The ID arrays are not aligned!"

    # 3. Glue them side-by-side (axis=1 means horizontally)
    matched_df = pd.concat([matched_cat, embeddings], axis=1)

    # 4. Drop any rows that have NaNs (if that was your original intent)
...
if __name__ == "__main__":
    '''
    data_files = {}
    model_no = "7655991_0" # spender I v2 seed = 42.
    for split in ["train", "test", "val"]:
        catalog, keep_cols = process_gswlc_catalog(
            catalog_dir=CATALOG_DIR
        )
        embeddings = process_embeddings(
            fp=f"{DATA_ROOT}/sdss_II/spender_I_flow_v2/embeddings/{model_no}",
            split=split
        )
