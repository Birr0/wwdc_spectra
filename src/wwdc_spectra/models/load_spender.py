...
class Spender(nn.Module):
    ...
    def init_network(m): # Is this covered with random number initialisations?
        ...
        # PReLU: reset the learnable slope (common default is 0.25)
...
class SpectrumEncoder(nn.Module):
    ...
    def forward(self, y):
        ...
        if (not self.training) and (a.requires_grad):
...
if __name__ == "__main__":
    ...
    data = SDSS_II(split="train")
    dataset = WWDCDataset(data, return_catalog=True)
    loader = WWDCDataLoader(datasets={"train": dataset, "test": dataset, "val": dataset}, batch_size=16)
    loader.setup()
    ...
    print("Init AE")
    ae = SpectrumAutoencoder(
        instrument,
        wave_rest,
        n_latent=10,
        act=(
            SpeculatorActivation(64), 
            SpeculatorActivation(256), 
            SpeculatorActivation(1024), 
    ...
    ae.to("cuda")
    X = torch.rand(1, 3921).to("cuda")
    c = ae(X, z=torch.tensor([0.1]).to("cuda"))
    print(c.shape)
    print(c.device)

    loss = torch.sum((c - X)**2, axis=1).mean()
    print(loss)
    print(loss.device)
    loss.backward()
    print("Finsished!")
    ...
    with initialize(version_base=None, config_path="../../../src/conf/"):
        cfg = compose(config_name="experiment/spender_I_flow/train")
        
    #train_dset = instantiate(cfg.data.train_dataset)
    loader = instantiate(cfg.data.loader)
    loader.setup()
    train_loader = loader.train_dataloader()

    model = instantiate(cfg.lightning_loader)
    print(model)
    for batch in train_loader:
        output = model.base_step(
            batch,
            partition="train"
        )
        break'''
