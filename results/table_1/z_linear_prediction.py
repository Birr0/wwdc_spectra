...
if __name__ == "__main__":
    ...
    p.add_argument("--base_model", type=str, default="spender_I", help="Base model: spender I/II") # Fixed default to match your if-statement
    ...
    # 2. Fit and Predict with a standard Linear Model
    ...
    # Updated directories to reflect the linear model
    os.makedirs(f"./linear_models_{base_model}_v2", exist_ok=True)
    os.makedirs(f"./linear_predictions_{base_model}_v2", exist_ok=True)

    joblib.dump(model, f"./linear_models_{base_model}_v2/{embed_type}_{idx}.joblib")

    # Create summary DataFrame
    df_results = pd.DataFrame(results_list)
    df_results.to_csv(f"./linear_predictions_{base_model}_v2/{embed_type}_{idx}.csv", index=False)