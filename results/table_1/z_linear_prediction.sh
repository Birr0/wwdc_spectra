#!/bin/bash
#SBATCH --job-name=z_linear_predictor
#SBATCH --output=linear_%A_%a.out
#SBATCH --error=linear_%A_%a.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python z_linear_prediction.py --idx "$idx" --embed_type "$embed_type" --base_model "$base_model"
echo "Job completed"